...
namespace Satellite
{
	Texture::Texture(SDL_Texture* texture, int width, int height)
		: texture(texture), width(width), height(height), tile_size(0)
	{ }

	Texture::Texture(SDL_Texture* texture, int width, int height, int tile_size)
		: texture(texture), width(width), height(height), tile_size(tile_size)
	{ }
	...
	{
		if (tile_size == 0) {
			std::cout << "ERROR - The current texture has a tileSize of 0, which means that it is not a tileSet and just a single image." << std::endl;
			return { 0, 0, 0, 0 };
		}
		...
		if (id < 0 || id > columns * rows) {
			std::cout << "ERROR - The id passed as parameter is not a valid id for current texture." << std::endl;
		}
